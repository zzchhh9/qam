"""Standalone eval for QAM-BCFast checkpoints. Same as eval_canonical_qam.py
but uses QAMBCFastAgent (extra bc_fast_alpha hyperparameter, identical eval path).
Usage:
    CUDA_VISIBLE_DEVICES=7 python eval_canonical_qam_bcfast.py \\
...
from agents.qam_bcfast import QAMBCFastAgent, get_config
config = get_config()
...
agent = QAMBCFastAgent.create(SEED, ex_obs, ex_act, config)
