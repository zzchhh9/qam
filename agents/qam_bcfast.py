...
class QAMBCFastAgent(flax.struct.PyTreeNode):
    """QAM variant: add direct BC flow-matching loss to actor_fast.

    Standard QAM trains actor_fast ONLY via adjoint matching (regression toward
    actor_slow + sigma^2/2 * adj). On sparse-reward chunked-action tasks
    (e.g. BiGym RemoveSandwich), the Q gradient is tiny (adj_mean ~ 4e-4/dim),
    so actor_fast learns to mimic actor_slow indirectly through an
    under-constrained MSE on (v_fast - v_slow) — and the (2/sigma)^2 weighting
    leaves the small-t portion of the ODE almost free. After ~30K offline
    steps actor_fast diverges from actor_slow and eval SR collapses to 0%.

    Fix: add a direct BC flow-matching loss on actor_fast (same form as the
    one actor_slow already gets). Now actor_fast has BC supervision AND Q
    correction. New hyperparameter `bc_fast_alpha` (default 1.0) weights it.
    """
    ...
    def actor_loss(self, batch, grad_params, rng):
        ...
        ## NEW: BC flow-matching loss on actor_fast (the eval policy).
        # Uses the same (x_t, t, vel) triplet as actor_slow so both networks see
        # identical BC supervision. Without this, actor_fast only has the weak
        # indirect MSE through adj_loss and drifts off the BC manifold.
        bc_fast_alpha = self.config.get("bc_fast_alpha", 1.0)
        if bc_fast_alpha > 0.0:
            pred_fast = self.network.select('actor_fast')(
                batch['observations'], x_t, t, params=grad_params
            )
            flow_loss_fast = jnp.mean(
                jnp.square(pred_fast - vel).mean(axis=-1) * batch["valid"][..., -1]
            )
            info["flow_loss_fast"] = flow_loss_fast
            total_fast_loss += bc_fast_alpha * flow_loss_fast
...
def get_config():
    config = ml_collections.ConfigDict(
        dict(
            agent_name='qam_bcfast',  # Agent name.
            ...
            bc_fast_alpha=1.0,  # NEW: weight for direct BC flow loss on actor_fast (the eval policy). 0 disables.
